...
namespace Baltin
{
    public class LayersEnumGenerator : EditorWindow
    {
        ...
        [MenuItem("Tools/Layers Enum Generator")]
        public static void ShowSettingsWindow()
        {
            var window = GetWindow<LayersEnumGenerator>("Layers Enum Generator");
            window.minSize = new Vector2(400, 200);
        }

        // Removed this method to remove "Generate Layers Enum" menu item
            ...
            using (var writer = new StreamWriter(outputPath))
            {
                ...
                for (int i = 0; i < layers.Length; i++)
                    if (!string.IsNullOrEmpty(layers[i]))
                    {
                        ...
                        writer.WriteLine($"        {sanitizedLayerName} = {i},    // {layers[i]}");
                    }
